...
class FirmwareWrapper(BaseController):
    ...
    def step(self, sim_time: float, action: np.ndarray):
        ...
        if self.verbose:
            ...
            SPHERE_URDF = (
                ...
                + "/../../envs/gym_pybullet_drones/assets/sphere.urdf"
            )
        ...
        while self.tick / self.firmware_freq < sim_time + self.ctrl_dt:
            ...
            if self.takeoff_sent:
                self.states += [
                    [self.tick / self.firmware_freq, cur_pos[0], cur_pos[1], cur_pos[2]]
                ]
            ...
            if self.SENSOR_DELAY:
                ...
                self.sensor_history = self.sensor_history[1:] + [
                    [body_rot.apply(cur_acc), cur_rotation_rates * self.RAD_TO_DEG]
                ]
            else:
    ...
    def _update_sensorData(self, timestamp, acc_vals, gyro_vals, baro_vals=[1013.25, 25]):
        ...
        """
        # Only gyro and acc are used in controller. Mag and baro used in state etimation (not yet
        # supported)
    ...
    def _powerDistribution(self, control_t):
        ...
        if self.QUAD_FORMATION_X:
            ...
            motor_pwms += [
                self._motorsGetPWM(
                    self._limitThrust(control_t.thrust + control_t.pitch + control_t.yaw)
                )
            ]
            motor_pwms += [
                self._motorsGetPWM(
                    self._limitThrust(control_t.thrust - control_t.roll - control_t.yaw)
                )
            ]
            motor_pwms += [
                self._motorsGetPWM(
